...
\todo{The 2 comes from the two copies of the term ${x}^{T}{A}^{T}Ah={h}^{T}{A}^{T}Ax$ (they are equal because they are $1\times 1$ and one the transpose of the other). Each of them contributes one $A^TAx$. Think about the scalar case, to have a different angle: the derivative of $x^2$ is $2x$, because $(x+h)^2=x^2+2hx+o(h)$.}
...
\begin{equation}
  \begin{split}
    ...
    & = {\bf \color{blue} {x}^{T}{A}^{T}Ax} + {x}^{T}{A}^{T}Ah + {h}^{T}{A}^{T}Ax + {\bf \color{red} {h}^{T}{A}^{T}Ah} {\bf \color{blue}- 2{b}^{T}Ax} - 2{b}^{T}Ah + {\bf \color{blue} {b}^{T}b}\\
...
We may choose $u = x-y$ \todo{missing}
